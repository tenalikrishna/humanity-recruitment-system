...
export async function registerRoutes(
  ...
  app.post("/api/forms/volunteer", async (req, res) => {
    try {
      ...
      // Also save to recruitment DB so it appears in the admin dashboard
      try {
        await adminStorage.createApplication({
          ...
          phone: data.phone,
          city: data.city,
          programInterest: data.projects.join(", "),
          notes: `Occupation: ${data.occupation} | Type: ${data.volunteerType.join(", ")}`,
          status: "pending",
          updatedAt: new Date(),
        });
      } catch (dbErr) {
        // Don't fail the request if DB write fails — Sheets is the source of truth
        console.error("Failed to save volunteer to recruitment DB:", dbErr);
      }
