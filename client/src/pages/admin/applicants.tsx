...
import { RequireAdminAuth, useAdminAuth } from "@/lib/adminAuth";
  ...
  useEffect(() => {
    async function load() {
      ...
      if (user?.role !== "screener") {
        const sRes = await fetch("/api/admin/screeners", { credentials: "include" });
                    ...
                    return (
                      <div className="flex items-center justify-between bg-blue-500/10 border border-blue-500/20 rounded-xl px-4 py-3">
                        ...
                        {asg.status === "completed" && (
                          <button
                            onClick={() => navigate(`/admin/interview/${selected.id}`)}
